# detector.py
# 目标检测器
#
# @author n1ghts4kura
# @date 25-12-6
#

import cv2
from dataclasses import dataclass
from ultralytics import YOLO
from typing import Optional


from src import config
from src import logger


@dataclass
class DetectionResult:
    """
    目标检测结果数据类
    """
    cls_id: int               # 目标类别ID 1 - 红色装甲板 2 - 蓝色装甲板
    confidence: float         # 目标置信度
    xywh: tuple[float, float, float, float]  # 目标边界框 (x_center, y_center, width, height)
    xywhn: tuple[float, float, float, float] # 归一化边界框 (x_center, y_center, width, height)


class Detector:
    """
    目标检测器
    """

    def __init__(self, model_path: str):
        self.model: Optional[YOLO] = None
        self.model_path = model_path
        self.enabled: bool = False


    def initialize(self) -> bool:
        """
        初始化检测器

        Returns:
            bool: 初始化是否成功
        """

        try:
            self.model = YOLO(model=self.model_path, task="detect")
            self.enabled = True
            return True
        except Exception as e:
            logger.error(f"装甲板检测器初始化失败: {e}")
            return False
    
    
    def detect(self, frame: cv2.typing.MatLike) -> list[DetectionResult]:
        """
        在给定帧中检测目标

        Args:
            frame (cv2.typing.MatLike): 输入图像帧

        Returns:
            list[DetectionResult]: 检测到的目标列表
        """

        if self.model is None:
            logger.error("目标检测器未初始化")
            return []

        results_list: list[DetectionResult] = []

        # 强制只读取第一个result (可能会发生意外 但是不太可能)
        result = self.model.predict(frame, verbose=False, device=config.AIMBOT_PREDICT_DEVICE)[0]
        result = result.cpu()  # 确保在CPU上处理

        # 如果没有检测到任何目标，返回空列表
        if result.boxes is None:
            return []

        # 解析检测结果
        for box in result.boxes: 
            cls_id = int(box.cls[0]) # 类别ID
            confidence = float(box.conf[0]) # 置信度
            # 边界框坐标 (x_center, y_center, width, height)
            x_center, y_center, width, height = box.xywh[0].tolist()
            x_center_n, y_center_n, width_n, height_n = box.xywhn[0].tolist()

            detection_result = DetectionResult(
                cls_id=cls_id,
                confidence=confidence,
                xywh=(x_center, y_center, width, height),
                xywhn=(x_center_n, y_center_n, width_n, height_n)
            )
            results_list.append(detection_result)

        return results_list


class AimbotDetector(Detector):
    """
    自瞄 装甲板目标检测器
    """

    def __init__(self):
        super().__init__(model_path=config.AIMBOT_MODEL_PATH)
